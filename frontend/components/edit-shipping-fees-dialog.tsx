...
export function EditShippingFeesDialog({ open, onOpenChange, canEdit }: { open: boolean; onOpenChange: (open: boolean) => void; canEdit: boolean }) {
  ...
  const [fees, setFees] = useState<{ [key: string]: { baseFee: number; additionalFee: number } }>({});
  ...
  const handleChange = (category: string, field: "baseFee" | "additionalFee", value: string) => {
    ...
    setFees((prev) => ({
      ...prev,
      [category]: { ...prev[category], [field]: num }
    }));
  };
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      await shippingFeeService.updateShippingFees(fees);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!canEdit || saving || loading} className="bg-brand-primary hover:bg-brand-primary/90">
